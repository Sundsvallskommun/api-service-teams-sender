...
public class MicrosoftGraphIntegration {
	...
	/**
	 * Sends a one-on-one Teams chat message from the configured sender user to the given recipient, creating the chat if
	 * it does not already exist. The sender member is bound by UPN (Graph's users('...') binding accepts both UPN and
	 * id), which saves a Graph round-trip per send.
	 */
	public void sendChatMessage(final String municipalityId, final String recipient, final String message) {
		...
		final String sender = tokenService.getSenderUser(municipalityId);
		final String recipientId = resolveUserId(graphClient, recipient);
		final String chatId = createOneOnOneChat(graphClient, sender, recipientId);
	...
	private String createOneOnOneChat(final GraphServiceClient graphClient, final String sender, final String recipientId) {
		final Chat chat = new Chat();
		chat.setChatType(ChatType.OneOnOne);
		chat.setMembers(List.<ConversationMember>of(asMember(sender), asMember(recipientId)));
	...
	private static AadUserConversationMember asMember(final String userIdOrUpn) {
		...
		member.setAdditionalData(Map.of("user@odata.bind", "https://graph.microsoft.com/v1.0/users('" + userIdOrUpn + "')"));
